...
import { ClienteEnt } from 'src/cliente/entities/cliente.entity';
...
async addProductoToFactura(nroFactura:number,idProducto:number,cantidad:number):Promise<void>{
  ...
  async update(id: number, facturaDto: FacturaDto): Promise<Factura> {
  ...
  // Cargar los productos por ID
  const productos = await this.productoRepository.findByIds(facturaDto.productos);

  if (productos.length !== facturaDto.productos.length) {
    throw new BadRequestException('Uno o más productos no existen');
  }

  // Crear un objeto actualizado con los productos cargados
  const facturaActualizada = this.facturaRepository.merge(facturaExistente, {
    ...facturaDto,
    productos,
  });

  return await this.facturaRepository.save(facturaActualizada);
}
